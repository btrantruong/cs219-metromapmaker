...
public class AppGUI {
    ...
    public static final boolean LABELED = true;
    ...
    public Button initPaneChildButton(Pane pane, String name, boolean enabled) {
     ...
     public Button initPaneChildButton2(Pane pane, String name, boolean enabled, boolean label) {
        // MAKE THE BUTTON
        Button button;
        if (label==true){
             button = makeButtonWithLabel(name, enabled);
        }
        else 
            button = makeButton(name, enabled);
        // PUT THE BUTTON IN THE PANE
        pane.getChildren().add(button);
    ...
    public Button makeButtonWithLabel(String name, boolean enabled) {
        PropertiesManager props = PropertiesManager.getPropertiesManager();

        String tooltipProperty = name + "_TOOLTIP";
        String labelName = props.getProperty(name);
        // NOW MAKE THE BUTTON
        Button button = new Button();
        button.setDisable(!enabled);
        Label label = new Label(labelName);
        if(labelName.equals("Set Image Background")){
            label.setPrefSize(75, 35);
        }
        else
            label.setPrefSize(60, 35);
